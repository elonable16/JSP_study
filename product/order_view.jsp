...
<%
	...
	try{
		sql = "select m.m_name, m.m_addr, m.m_phone, m.m_date, sum(p.p_price* acs.as_cnt)as total ";
		sql +="from member m, account_main am, account_sub acs, product p ";
		sql +="where m.m_id = ? and m.m_id = am.m_id and am.am_code = acs.am_code and acs.p_code = p.p_code group by m.m_name, m.m_addr, m.m_phone, m.m_date";
		...
		<div id="section">
			<div id="article">
				<table>
					...
					<tr>
						<td class="td_title_base">가입 일자</td>
						<td class="td_contents"><%=m_date%></td>
					</tr>
					<tr>
						<td class="td_title_base">구매 누적 금액</td>
						<td class="td_contents"><%=comma.format(total)+"원"%></td>
					</tr>
					<tr>
...
%>
					<tr>
						<td colspan="2" class="td_title_long">주문 금액 합계 : <%=order_total %></td>
					</tr>
				</table>
