...
<%
	...
	String bs_update_num = request.getParameter("bs_num");
						...
						<tr>
							<td colspan="2" class = "td_title_long">
								<p>
									...
									<a href = "./update.jsp?guest_num=<%=bm_num%>">[수정]</a>
								</p>
							</td>
						</tr>
<%
					...
					while(rs.next()){
						...
						if(Integer.parseInt(bs_update_num) == bs_num){
%>
							<tr>
								<td colspan="2" class = "td_bs_contents">
									<form action ="./update_sub_ok.jsp" method="post">
										<input type="hidden" name="bm_num" value="<%=bm_num %>">
										<input type="hidden" name="bs_num" value="<%=bs_num %>">
										<textarea cols = "85", rows= "10" name="bs_contents"><%=bs_contents%></textarea>
										<br>
										<input type="submit" value="수정완료">
										<button><a href="./view.jsp?bm_num=<%=bm_num %>">수정취소</a></button>
									</form>
								</td>
							</tr>
<%
